...
class CALMID_v2(WrapperEnsemble, Classifier):
    def __init__(
        self,
    ...
    def learn_one(self, x, y):
        ...
        if labelling:
            if y not in self.label_to_index:
                ...
                self.learning_queues.append(LIFO(max_size=self.sizesam))
                [row.append(self.theta) for row in self.amt_matrix]
                self.amt_matrix.append([self.theta for _ in range(len(self.label_to_index))])
                self.sizesam = ceil(self.sizelab * self.epsilon / len(self.label_to_index))
